...
from ssh_scanner import format_ssh_findings, run_ssh_scan
...
def main() -> int:
    parser = argparse.ArgumentParser(
        add_help=False,
        description="Запуск сканеров cron, sudo и ssh (по ключам или все по умолчанию).",
    )
    parser.add_argument(
        "--help",
        action="help",
    ...
    parser.add_argument(
        ...
        "--ssh-scan",
        action="store_true",
        help="Включить сканирование SSH.",
    )
    parser.add_argument(
        "--cron-paths",
        "--paths",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Дополнительные файлы логов SSH для проверки.",
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    run_ssh = args.ssh_scan
    if not (run_cron or run_sudo or run_ssh):
        run_cron = True
        run_sudo = True
        run_ssh = True
    ...
    if run_ssh:
        ssh_findings = run_ssh_scan(extra_paths=args.ssh_logs)
        sections.append(("SSH", format_ssh_findings(ssh_findings)))
