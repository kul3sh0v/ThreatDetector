...
def run_scan(extra_paths: Optional[List[str]] = None) -> List[Dict[str, str]]:
    findings: List[Dict[str, str]] = []
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Сканер cron: ищет потенциально вредоносные инъекции в заданиях cron.",
    )
    parser.add_argument(
        "--paths",
        nargs="*",
    ...
    if os.name == "nt":
        print("Сканер предназначен для Linux cron. Windows Task Scheduler не поддерживается.", file=sys.stderr)
        return 2

    findings = run_scan(extra_paths=args.paths)
